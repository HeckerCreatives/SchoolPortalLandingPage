...
export const ChatBox: React.FC = () => {
  ...
  const createConversation = async () => {
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_URL}/supportconversation/createconversation`,
        {
  ...
  setChatData((prevData) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_URL}/supportmessage/anonymoussendmessage`,
        {
          conversationid: id,
          userid: '',
          message: filter.clean(inputText),
          ...
          <div className="p-4 flex-1 h-full overflow-y-auto scrollbar">
            {chatData?.messages.map((message, index) => (
              ...
              {message.type === 'System' ? (
                ...
                key={index}
                className={`mb-4 ${
                  message.sender.userType === 'Anonymous' ? 'text-right' : 'text-left'
                }`}
              >
                <p
                  className={`inline-flex flex-col p-2 rounded-lg ${
                    message.sender.userType === 'Anonymous'
